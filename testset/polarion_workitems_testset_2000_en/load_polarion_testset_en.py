...
DATA_FILE = Path("polarion_workitems_testset_2000_en.jsonl")
COLLECTION_NAME = "polarion_workitems_en"
    ...
    with path.open("r", encoding="utf-8") as file:
        for line_number, line in enumerate(file, start=1):
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid JSON in line {line_number}"
                ) from exc
...
def main() -> None:
    ...
    for batch in chunks(iter_workitems(DATA_FILE), BATCH_SIZE):
        ...
        imported += len(batch)
        print(f"Imported {imported} workitems")
